...
@interface BaseMapCluster () {
    BaseMapCluster * _leftChild;
    BaseMapCluster * _rightChild;
    MKMapRect _mapRect;
...
@interface BaseMapCluster (Private)
- (MKMapRect)_mapRect;
...
- (id)initWithAnnotations:(NSArray *)annotations atDepth:(NSInteger)depth inMapRect:(MKMapRect)mapRect gamma:(double)gamma clusterTitle:(NSString *)clusterTitle showSubtitle:(BOOL)showSubtitle {
    ...
    if (self) {
        ...
        if (annotations.count == 0) {
            ...
            double XSum = 0.0;
            double YSum = 0.0;
            for (BaseMapPointAnnotation * annotation in annotations) {
            ...
            if (fabs(sumXY)/annotations.count > BaseMapClusterDiscriminationPrecision) {
                aX = sumXY;
            ...
            if (fabs(sumXsquared)/annotations.count < BaseMapClusterDiscriminationPrecision || fabs(sumYsquared)/annotations.count < BaseMapClusterDiscriminationPrecision) { // all X and Y are the same => same coordinates
            ...
            for (BaseMapPointAnnotation * annotation in leftAnnotations) {
                const MKMapPoint point = annotation.mapPoint;
                if (point.x > XMax) {
                    XMax = point.x;
                }
            ...
            for (BaseMapPointAnnotation * annotation in rightAnnotations) {
                const MKMapPoint point = annotation.mapPoint;
                if (point.x > XMax) {
                    XMax = point.x;
                }
            ...
            _leftChild = [[BaseMapCluster alloc] initWithAnnotations:leftAnnotations atDepth:depth+1 inMapRect:leftMapRect gamma:gamma clusterTitle:clusterTitle showSubtitle:showSubtitle];
            _rightChild = [[BaseMapCluster alloc] initWithAnnotations:rightAnnotations atDepth:depth+1 inMapRect:rightMapRect gamma:gamma clusterTitle:clusterTitle showSubtitle:showSubtitle];
...
+ (BaseMapCluster *)rootClusterForAnnotations:(NSArray *)initialAnnotations gamma:(double)gamma clusterTitle:(NSString *)clusterTitle showSubtitle:(BOOL)showSubtitle {
    // KDTree
    ...
    for (BaseMapPointAnnotation * annotation in initialAnnotations) {
    ...
    BaseMapCluster * cluster = [[BaseMapCluster alloc] initWithAnnotations:initialAnnotations atDepth:0 inMapRect:boundaries gamma:gamma clusterTitle:clusterTitle showSubtitle:showSubtitle];
...
- (NSArray *)find:(NSInteger)N childrenInMapRect:(MKMapRect)mapRect {
    ...
    while (clusters.count + annotations.count < N && clusters.count > 0 && clustersDidChange) {
        ...
        for (BaseMapCluster * cluster in clusters) {
            for (BaseMapCluster * child in [cluster children]) {
                if (child.annotation) {
...
@implementation BaseMapCluster (Private)
...
- (void)_cleanClusters:(NSMutableArray *)clusters fromAncestorsOfClusters:(NSArray *)referenceClusters {
    ...
    for (BaseMapCluster * cluster in clusters) {
        for (BaseMapCluster * referenceCluster in referenceClusters) {
